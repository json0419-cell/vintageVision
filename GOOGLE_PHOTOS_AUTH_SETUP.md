...
## Overview
VintageVision now supports Google Photos login, allowing users to quickly sign in with their Google account and access their photo library.
...
Add the following configuration to `backend/config.env.example`:
...
### User Permissions
- **Basic Profile Access**: Get user's name, email, and profile picture
...
3. After user authorization, the system automatically creates or updates user account
...
- Support for linking existing user accounts
